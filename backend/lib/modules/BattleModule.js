			...
			const interval = setInterval(async () => {
				if (++counter > battle.cases.length) {
					...
					for (let [steamID, cases] of Object.entries(battle.openedCases)) {
						for (let c4se of cases) {
							sums[steamID] = sums[steamID] + c4se.item.suggested_price || c4se.item.suggested_price
							itemsValue = itemsValue + c4se.item.suggested_price || c4se.item.suggested_price
						}
					}
					...
					let itemsSorted = _.sortBy(items, 'suggested_price').reverse();
					...
					for (let i = 0; i < itemsSorted.length; i++) {
						...
						if ((item.suggested_price + taxed) <= maxTax) {
							console.log(item.name + ' - ' + item.suggested_price);
							taxed += item.suggested_price;
						...
						this.tradeAPI.sendOffer(winner.opData.long_url, userWinnings, [], async (err, offerID) => {
							...
							if(taxedItemIDs.length > 0) {
								try {
									...
									await db.r
										...
										.replace(row => {
											return row.branch(
												{
													date,
													revenue: row('revenue').add(total_value.usd),
													amount: row('amount'),
													value: row('value'),
													topDay: row('topDay'),
												},
									...
									console.log(total_value, `SOLD TAXED ITEMS FOR - $${(parseInt(total_value.usd) / 100).toFixed(2)}`)
								} catch (e) {
												...
												return row.branch(
													{
														...
														topDay: row('topDay')
															.union(items)
															.orderBy(db.r.desc(row => row('case')('item')('suggested_price')))
															.limit(30),
														revenue: row('revenue'),
													},
													{
										...
										/*const schemaID = results.changes[0].new_val.schemaID.toString();
												  ...
												  db.r.table('users').get(socket.user.steamID).update({
													  stats: {[schemaID]: {
														  ...
														  openSum: db.r.row('stats')(schemaID)('openSum').default(0).add(results.changes[0].new_val.item.suggested_price)
													  }},
													  canChat: true
												  }).run(err => {
